...
if [ "$SKIP_RUN" = false ]; then
    ...
    while IFS= read -r exe; do
        ...
        RUSTFLAGS="-C instrument-coverage" \
            cargo test -p zuicchini --tests --no-run --message-format=json 2>/dev/null \
    ...
    # Record timestamp and git state for provenance.
    {
        ...
        echo "commit: $(git -C "$WORKSPACE_ROOT" rev-parse --short HEAD 2>/dev/null || echo unknown)"
        echo "dirty: $(git -C "$WORKSPACE_ROOT" diff --quiet 2>/dev/null && echo no || echo yes)"
    } > "$OUTDIR/metadata.txt"
