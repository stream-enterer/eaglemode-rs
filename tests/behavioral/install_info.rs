...
use eaglemode_rs::emCore::emInstallInfo::{emGetConfigDirOverloadable, emGetInstallPath, InstallDirType, InstallInfoError};
...
fn config_dir_overloadable_versions_match() {
    let tmp = std::env::temp_dir().join("eaglemode_test_install_info_match");
...
fn config_dir_overloadable_versions_mismatch() {
    let tmp = std::env::temp_dir().join("eaglemode_test_install_info_mismatch");
    let host_dir = tmp.join("etc").join("testprj");
    let user_dir = tmp.join("userconfig").join("testprj");
    std::fs::create_dir_all(&host_dir).unwrap();
    std::fs::create_dir_all(&user_dir).unwrap();
...
fn config_dir_overloadable_no_user_version() {
    let tmp = std::env::temp_dir().join("eaglemode_test_install_info_no_user");
    let host_dir = tmp.join("etc").join("testprj");
    let user_dir = tmp.join("userconfig").join("testprj");
    std::fs::create_dir_all(&host_dir).unwrap();
    std::fs::create_dir_all(&user_dir).unwrap();
...
fn config_dir_overloadable_with_sub_dir() {
    let tmp = std::env::temp_dir().join("eaglemode_test_install_info_subdir");
    let host_dir = tmp.join("etc").join("testprj");
    std::fs::create_dir_all(&host_dir).unwrap();
    std::fs::write(host_dir.join("version"), "1.0.0\n").unwrap();
