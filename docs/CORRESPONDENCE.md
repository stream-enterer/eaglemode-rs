...
- emAbsoluteFileModelClient added to emFileModel.rs. Uses
  Weak<RefCell<T>> instead of raw pointer + emRef (DIVERGED).

emCore types stress-tested by emStocks:
- emCrossPtr<T> — explicit invalidation for dialog and record tracking
- emRecFileModel — file I/O state machine for .emStocks data
- emConfigModel — configuration persistence for emStocksConfig
...
- emStocksRec: Rust struct fields use snake_case. Method names preserve
  C++ names. InterestRec replaced by Interest enum with explicit
  deprecated-identifier methods.
- emStocksFileModel: composition instead of multiple inheritance.
  Save timer uses std::time::Instant (not emTimer/scheduler).
